...
AR 对比共用当前 WebXR 渲染器及帧缓冲，通过 scissor 分别绘制左右模型；不要为模型创建额外 WebGL 渲染器或 DOM canvas，以免重复上传高斯纹理并增加移动端显存压力。裁剪坐标取自 XR viewport，并转换为渲染器的逻辑像素。桌面对比仍使用离屏纹理合成。

单模型与双模型加载互斥；双模型按 A、B 顺序加载并显示进度，替换前等待旧 Viewer 释放。运行 `node tests/ply-ar-split.test.js` 检查裁剪、像素比和异常后的状态恢复。实际 AR 帧率、地面放置和拖动效果仍需支持 WebXR 的手机通过 HTTPS 验收。
